...
class WeatherDBRepository extends BaseDBRepository
{
    public function findWeatherByUserId($userId) {
        $query = "select time, temperature, wind_direction_id, wind_speed, tide_type, tide_power from user_weather where user_id = :user_id and DATE_FORMAT(time, '%Y.%m.%d.%H') = DATE_FORMAT(now(), '%Y.%m.%d.%H')";
        ...
        try {
            ...
            if ($result) {
                $result = new Weather($result["time"], $result["temperature"], $result["wind_direction_id"]
                    , $result["wind_speed"], $result["tide_type"], $result["tide_power"]);
            }
        } catch(Exception $e) {
    ...
    public function insertWeatherByUserId($userId, $result) {
        $query = "insert into user_weather values (:user_id, :time, :temperature, :wind_direction_id, :wind_speed, :tide_type, :tide_power)";
