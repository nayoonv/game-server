...
// Check whether a detailed logd is wanted.
...
Options:
    ...
    --show-[all|php|skip|clean|exp|diff|out]
                ...
                get written independent of the logd format, however 'diff' only
...
";
	...
	$log_filename      = $temp_dir . DIRECTORY_SEPARATOR . $main_file_name . 'logd';
	...
	if ($leak_check) {
		...
		if ($valgrind_version >= 330) {
			/* valgrind 3.3.0+ doesn't have --logd-file-exactly option */
			$cmd = "valgrind -q --tool=memcheck --trace-children=yes --logd-file=$memcheck_filename $cmd";
		} else {
			$cmd = "valgrind -q --tool=memcheck --trace-children=yes --logd-file-exactly=$memcheck_filename $cmd";
		}
...
---- FAILED
", FILE_BINARY) === false) {
			error("Cannot create test logd - $log_filename");
